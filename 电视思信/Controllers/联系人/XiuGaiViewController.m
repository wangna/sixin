...
//    [window viewWithTag:123].hidden=YES;
    ...
    lable1.hidden=YES;
    [lable1 setPlaceholder:@"新密码"];
    [lable1 setSecureTextEntry:YES];
    ...
    lable2.hidden=YES;
    [lable2 setPlaceholder:@"确定密码"];
    [lable2 setSecureTextEntry:YES];
    ...
    if (self.index==5) {
        [lable setPlaceholder:@"旧密码"];
        [lable setSecureTextEntry:YES];
        lable1.hidden=NO;
        lable2.hidden=NO;
...
//    if ([lable.text length]>0&&[lable.text length]<9) {
        ...
        if (self.index==2) {
            if (![self kongge:lable.text]) {
                [SVProgressHUD showAbnormalThenDismiss:@"请输入昵称"];
                return;
            }
            if ([lable.text length]>0&&[lable.text length]<9) {
    ...
    if (self.index==5) {
        ...
        }else if ([lable.text isEqualToString:lable1.text]) {
            [SVProgressHUD showAbnormalThenDismiss:@"旧密码和新密码不能相同"];
            return;
...
//        [request setURL:[NSURL URLWithString:@"http://119.44.220.4/edituserinfo"]];
    ...
    [request setURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@edituserinfo?",url]]];
